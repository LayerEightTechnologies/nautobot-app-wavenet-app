...
class LoadBuildings(Job):
    ...
    building_name = IntegerVar(
        description="Building Name",
        label="Building Name",
        required=True,
    )

    buildings_dropdown = ChoiceVar(description="Select a building", label="Building", choices=[])

    def __init__(self, *args, **kwargs):
        """Initialise the job."""
        super(LoadBuildings, self).__init__(*args, **kwargs)
        self.fields["buildings_dropdown"].choices = self.fetch_buildings_list()
    ...
    def fetch_buildings_list(get_api_token=get_api_token):
        ...
        response = requests.get(
            "https://n8n.gateway.wavenet.co.uk/webhook/gateway-m2m-credential", headers=headers, timeout=60
        )
        if response.status_code == 200:
            data = response.json()
            dropdown_values = [(item["id"], item["name"]) for item in data]
