...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_LAYER8_APP_xxx
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.layer8_app.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.layer8_app.compose_http_timeout,
        "NAUTOBOT_VER": context.layer8_app.nautobot_ver,
        "PYTHON_VER": context.layer8_app.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.layer8_app.project_name}",
        f'--project-directory "{context.layer8_app.compose_dir}"',
    ]

    for compose_file in context.layer8_app.compose_files:
        compose_file_path = os.path.join(context.layer8_app.compose_dir, compose_file)
...
def run_command(context, command, **kwargs):
    ...
    if is_truthy(context.layer8_app.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.layer8_app.python_ver}...")
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations layer8_app"
...
@task(
    help={
        "version": "Version of Layer8 App to generate the release notes for.",
    }
)
...
def pylint(context):
    ...
    command = 'pylint --init-hook "import nautobot; nautobot.setup()" --rcfile pyproject.toml layer8_app'
    run_command(context, command)
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'layer8_app/*' --omit *migrations*"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.layer8_app.local):
